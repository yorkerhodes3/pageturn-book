...
| V3-405 | P1 | In progress (implementation verified) | Replaced the old single-turn check with 5 warm-ups plus 30 same-page measured turns on Plurality `3-2` with 20 exact notes and local-routed book-toned media, then optimized the active frame path and review-hardened cache/accessibility/evidence checks. A clean `b6e923e` V3-425 attempt retained all 1,230 intervals but failed on the contended host at 31.3 ms p95 with 10 long tasks (140 ms maximum). Three retained clean passing runs remain required; dirty-tree tuning passes do not close the item. |
