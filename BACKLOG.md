...
| V3-405 | P1 | In progress (implementation verified) | Replaced the old single-turn check with 5 warm-ups plus 30 same-page measured turns on Plurality `3-2` with 20 exact notes and local-routed book-toned media, then optimized the active frame path and review-hardened cache/accessibility/evidence checks. A post-review dirty-tree run passed at 21.8 ms p95 with no long tasks. Three retained clean-commit runs remain before closure; broader promotion still requires V3-425. |
