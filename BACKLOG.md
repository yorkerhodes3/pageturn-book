...
| V3-404 | P1 | In progress | Complete: moved the 14 reviewed figures from the demo catalog into shared validated manifest/CLI metadata and deterministic hashing. Remaining: generate measurement-driven responsive variants and complete provenance/editorial mapping across the library. |
| V3-405 | P1 | In progress (implementation verified) | Replaced the old single-turn check with 5 warm-ups plus 30 same-page measured turns on Plurality `3-2` with 20 exact notes and local-routed book-toned media, then optimized the active frame path and review-hardened cache/accessibility/evidence checks. The reveal-order correction restored a preallocated destination clip and passed a dirty-tree 4x run at 20.5 ms p95 with no long tasks. A clean `b6e923e` attempt still failed on the contended host at 31.3 ms p95 with 10 long tasks (140 ms maximum). Three retained clean passing runs remain required; dirty-tree tuning passes do not close the item. |
| V3-406 | P2 | Blocked | Obtain explicit rights metadata for the 26 unmapped Plurality figure links before enabling image rendering. They remain ordinary links; V3-424 does not infer permissions. |
...
| V3-413 | P2 | Complete | Migrated beta data into the resolved/unresolved version-2 IndexedDB schema; enforced the per-edition data cap; added transactional current-edition/all-edition bookmark-and-annotation deletion plus validated JSON backup/import with previewed merge/replace/conflict behavior; and completed the personal-data assistive-technology review. |
...
| V3-415 | P1 | Complete | Extracted typed material, paper, typography, binding, gutter, edge, and fold configuration; added seven live Style presets and session-only test controls; moved image display/style into compact Graphic handling and Typography boxes stacked in Style. |
...
| V3-418 | P1 | Complete | Added the public versioned exact-range selector kernel, Unicode/code-point source ranges across responsive paragraph/list splits, deterministic SHA-256 checksum and compact token codec, edition-bound anchor/Text Fragment URLs, exact recipient highlighting after bounded chapter loading, safe unresolved fallback, and history-race protection. |
| V3-419 | P1 | Complete | Added the contextual Copy, Share, Highlight, and Annotate action toolbar with exact-range validation and persistence, adaptive pointer/touch docking and collision avoidance, keyboard entry/roving focus, truthful clipboard fallback, Undo, capability gating, lifecycle teardown, and no redundant header Share action. |
| V3-420 | P1 | Complete | Added exact commenting-record margin editing, deterministic collision-managed handwritten outer-margin previews and accessible grouping, responsive markers and bottom sheets, exact resize/reload repositioning, inert page-turn clones, visibility/readable-font preferences, host appearance controls, and Explore edit/delete/export integration. |
| V3-421 | P1 | Complete | Added fail-closed publication share policy validation, an accessible exact-payload composer, bounded deterministic lazy Canvas 2D book clipping, exact File capability checks, text/image copy and download/open fallbacks, lifecycle cancellation, hosted policies, and privacy/accessibility/browser coverage. |
| V3-422 | P1 | Complete | Added the public host resolver and deterministic registry helper, canonical URL/DOI/ISBN and pinned-revision resolution, explicit course ambiguity, fail-closed local-rights validation, hosted reviewed mappings, local-edition-first and no-fetch external cards, canonical/provenance actions, cancellation, and accessibility/browser coverage. |
| V3-423 | P2 | Complete | Added strict public provider validation and exact origin/path matching, consent-only lazy iframe loading, sandbox/referrer/minimal Permissions Policy controls, nonce/origin/source/schema-validated cooperative readiness or honest bounded timeout, persistent direct-link fallback, fresh explicit retry, lifecycle teardown, controlled local fixtures, and security/browser coverage. |
| V3-424 | P1 | Complete (reviewed set) | Edition `2026-09` adds manifest integrity, verified edition-local bytes, caption/placement selector identity, exact anchor validation, source-aware URL resolution, complete rights/provenance gates, chapter-scoped placement uniqueness, and runtime original/book-toned/monochrome/duotone treatments for 3 Ethical AI and 11 rights-reviewed Plurality figures. `exportPermitted` remains policy-only with no source-image export/share path. V3-404 variants and the separate V3-406 set remain open. |
| V3-425 | P1 | Complete automated / promotion withheld | Added the reproducible 5-warm-up/30-run Chromium interaction harness, raw clean V3-405 evidence, provenance-validated commit-bound current/baseline builds, memory/network/lazy-boundary reporting, and focused root/Pages accessibility, static, embedded, Permissions Policy, privacy, rights, and lifecycle tests. Both full browser matrices pass 143/143. Fourteen of fifteen numeric gates pass, including selection at 61.1 ms p95; the verified initial V3 route grew 34,010 B gzip (>5 KiB). V3-405 clean evidence failed under contention, native/embedder cleanup was not established, and Firefox/Safari/iOS/Android plus manual NVDA/VoiceOver/TalkBack were not executed. See [the decision record](./V3-READER-INTERACTION-PROMOTION.md). |
| V3-426 | P1 | Ready | Extract personal storage, exact-target interaction, marginalia, share/source, and media UI integration behind behavior-preserving lazy boundaries until the disabled-feature public V3 route is no more than 5 KiB gzip above `c1cf0f8`. Do not defer the whole reader or delay first readable layout merely to move bytes. Re-run the validated detached-baseline harness and both full browser matrices. |
