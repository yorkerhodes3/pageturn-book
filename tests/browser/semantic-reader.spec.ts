...
test("presents V3 as the supported reader from the landing page", async ({
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: "PageTurn Book V3" }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: "Read with PageTurn V3" }),
  ).toHaveAttribute("href", "./v3/");
  await expect(
    page.getByRole("link", { name: "Open clean SDK example" }),
  ).toHaveAttribute("href", "./sdk/");
  await expect(
    page.getByRole("link", { name: "Enter the V3 library" }),
  ).toHaveAttribute("href", "./shelf/");
  await expect(
    page.getByRole("link", { name: "Compare V1, V2, and V3" }),
  ).toHaveAttribute("href", "./compare/");
  await expect(
    page.getByRole("link", { name: "Open pinned V1 fallback" }),
  ).toHaveAttribute("href", "./legacy/");
  await expect(
    page.getByRole("link", { name: "View V3 functionality dashboard" }),
...
test("documents implemented and planned capabilities on the dashboard", async ({
  ...
  await expect(
    page.getByRole("link", { name: /Open the catalog-free example/ }),
  ).toHaveAttribute("href", "../sdk/");
});
...
test("shows V1, V2, and V3 in the comparison view", async ({ page }) => {
  ...
  const v3 = page.frameLocator('iframe[title="V3 supported PageTurn reader"]');
...
test("pulls a shelf volume into the semantic reader", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await page
    .getByRole("link", { name: "Open V2 compatibility reader" })
    .click();
...
test("opens the full V3 semantic book from the shelf", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await page
    .getByRole("link", { name: "Read with PageTurn V3" })
    .click();
